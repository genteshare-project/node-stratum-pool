...
var generateOutputTransactions = function(poolRecipient, recipients, rpcData){
    ...
    /* BZXT bznode fix*/
    if (rpcData.bznode && rpcData.bznode_payments_started) {
        if (rpcData.bznode.payee) {
            var payeeReward = 0;

            payeeReward = rpcData.bznode.amount;
            reward -= payeeReward;
            rewardToPool -= payeeReward;

            var payeeScript = util.addressToScript(rpcData.bznode.payee);
            txOutputBuffers.push(Buffer.concat([
                util.packInt64LE(payeeReward),
                util.varIntBuffer(payeeScript.length),
                payeeScript
